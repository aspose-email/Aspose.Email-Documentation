---
title: "Mudanças na API Pública no Aspose.Email 5.3.0"
url: /pt/java/public-api-changes-in-aspose-email-5-3-0/
weight: 140
type: docs
---

A seguir está uma lista de quaisquer alterações feitas na API pública, como membros adicionados, renomeados, removidos ou descontinuados, bem como quaisquer alterações não compatíveis com versões anteriores feitas no Aspose.Email para Java. Se você tiver preocupações sobre qualquer alteração listada, por favor, levante-a no fórum de suporte do Aspose.Email.
## **APIs Adicionadas:**
- Classe `SyncFolderResult`
- Classe `TextFormat`
...
- Propriedade `Exchange.SyncFolderResult.getChangedItems()`
- Propriedade `Exchange.SyncFolderResult.getDeletedItems()`
- Propriedade `Exchange.SyncFolderResult.getNewItems()`
- Propriedade `Exchange.SyncFolderResult.getReadFlagChanged()`
- Propriedade `Exchange.SyncFolderResult.getSyncState()`
- Propriedade `Contact.getNotesFormat(), Contact.setNotesFormat(/**TextFormat**/int value)`
- Propriedade `MailMessage.isEncrypted()`
- Propriedade `MailMessage.getReadReceiptTo(), MailMessage.setReadReceiptTo(MailAddressCollection value)`

- Propriedade `MapiMessage.getReadReceiptRequested(), setReadReceiptRequested(boolean value)`
